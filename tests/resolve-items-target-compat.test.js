...
context.window.cargarJSON = async (url) => {
  if (url === '/data/catalog.json') {
    return { items: [{ id: 'item-1', title: 'Item 1' }] };
  }
  return null;
};
...
(async () => {
  ...
  if (!resolved || resolved.length !== 1 || resolved[0].id !== 'item-1') {
    console.error('FAIL', JSON.stringify(resolved));
    process.exit(1);
  }

  console.log('PASS', JSON.stringify(resolved));
})();
